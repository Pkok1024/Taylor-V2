...
const handler = async (m, {
  ...
  try {
    ...
    if (isNaN(urutan)) {
      return m.reply("Input query!\n*Example:*\n.instapost [nomor]|[query]\n\n*Pilih angka yg ada*\n" + input_data.map((item, index) => `*${index + 1}.* https://www.instagram.com/p/${item.shortcode}`).join("\n"));
    }

    if (urutan > input_data.length) {
      return m.reply("Input query!\n*Example:*\n.instapost [nomor]|[query]\n\n*Pilih angka yg ada*\n" + input_data.map((item, index) => `*${index + 1}.* https://www.instagram.com/p/${item.shortcode}`).join("\n"));
    }

    const outpic = input_data[urutan - 1].picture;
    const outcap = input_data[urutan - 1].description;
    const outcode = input_data[urutan - 1].shortcode;
