...
const fetchStickers = async (query) => {
  const url = `https://api.telegram.org/bot${BOT_TOKEN}/getStickerSet?name=${encodeURIComponent(query)}`;
  try {
    ...
    const stickerRequests = await Promise.all(stickers.map(async (sticker) => {
      const fileResponse = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/getFile?file_id=${sticker.file_id}`);
...
const handler = async (m, {
  ...
  if (!query) {
    return m.reply('*❗ Masukan tidak sesuai.*\nGunakan format yang benar: *' + usedPrefix + 'stickertele [query]* atau *' + usedPrefix + 'stickertelegram [query]|[angka]* atau *' + usedPrefix + 'telesticker [query]|all* atau *' + usedPrefix + 'telegramsticker [query]|random*');
  }
