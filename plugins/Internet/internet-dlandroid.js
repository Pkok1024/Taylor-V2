...
const handler = async (m, { conn, args, usedPrefix, text, command }) => {
  ...
  if (lister.includes(feature)) {
    if (feature === 'search') {
      ...
      await m.reply(wait);
      try {
        const res = await searchDlandroid(inputs[0]);
        const teks = res
          .map((item, index) => {
...
🔗 *Link:* ${item.downloadLink}`;
    ...
    if (feature === 'app') {
      ...
      try {
        const resl = await getDlandroid(inputs[0]);
...
const searchDlandroid = async query => {
